RSpec.describe 'Lost update versus a wrongly ordered repeatable read transaction' do
  around do |example|
    execute <<~SQL
      CREATE TABLE events (
        id text NOT NULL,
  ...
  specify <<-DESC.lstrip do
    Lost update anomaly still happens if only one transaction uses repeatable read isolation
    and that transaction is not the one that writes last;
    Bob's commit goes through first and since Alice's transaction level
    is read committed, her transaction does not detect the serialization failure
  DESC
