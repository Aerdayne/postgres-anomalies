RSpec.describe 'Non-repeatable read versus repeatable read isolation level' do
  around do |example|
    execute <<~SQL
      CREATE TABLE events (
        id text NOT NULL,
  ...
  specify <<-DESC.lstrip do
    Non-repeatable read anomaly is avoided;
    Bob sees the same version of Alice's booking throughout his transaction;
    all reads in his transaction see the same snapshot of the database
  DESC
    ...
    expect(buffer[:second_booking_reading]).to eq([1])
  end
end
